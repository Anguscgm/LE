...
def ego_Adj_NS_evaluation(A_orig,rho=0.5): 
    ...
    # norm_NS = np.linalg.norm(Aout_ego-P_hat,ord='fro')
    # rel_err_NS = norm_NS/np.linalg.norm(Aout_ego,ord='fro')
    # rel_err_full_NS = np.linalg.norm(A_orig-A_impute,ord='fro')/np.linalg.norm(A_orig,ord='fro')

    #return auc, fpr, tpr,density_NS, eigen_NS, PR_AUC, precision, recall, cc_NS, norm_NS, rel_err_NS
    return auc, fpr, tpr, PR_AUC, precision, recall, runtime
...
def ego_nonuniform_Adj_NS_evaluation(A_orig,rho=0.5,n_ratio=np.array([.33,.34,.33]),sample_ratio=np.array([1.5,1,.5])): 
