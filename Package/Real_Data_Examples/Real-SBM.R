...
for(j in 1:length(rho.seq)){
  ...
  rho.result <- foreach(i = 1:M)%dopar%{
    ...
    k.seq <- rep(NA,20)
    for(k in 1:20){
      k.seq[k] <- suppressMessages(Block.egocentric.CV(A,train.index,
                                                       Kmax=min(ceiling(SR+10),
                                                                ceiling(n^(1/2)))))
    }
    ...
    start.time = Sys.time()
    block.fit <- suppressMessages(Block.egocentric.fit(A,train.index,K=kblock))
