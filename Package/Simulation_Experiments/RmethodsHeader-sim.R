...
real.sample <- function(A,rho,deg){
  # P <- P*deg/mean(rowSums(P))
  # A <- net.gen.from.P(P)
  n <- nrow(A)
  sample.n <- ceiling(n*rho)
...
nonuniformbyA.sample <- function(P,rho,deg,n.ratio = c(.33,.34,.33),
                              sample.ratio= c(1.5,1,.5)){
  if (length(n.ratio)!=length(sample.ratio)){
    stop("n.ratio and sample.ratio must have the same legnth.")
  }
...
ego.perf <- function(A.impute,A.true,P,A.impute.out,A.true.out,P.out){
    ...
    if((sum(A.impute.out)==0)||(length(unique(as.numeric(A.true.out)))<2)){
      ...
      # cc.err <- abs(cc.Phat-cc.P)/cc.P
          ...
          tmp.norm <- norm(P.out-A.impute.out,"F")
          tmp.rel.err <- norm(P.out-A.impute.out,"F")/norm(P.out,"F")
          tmp.rel.err.full <- norm(P-A.impute,"F")/norm(P,"F")
          tmp.mse=norm(P.out-A.impute.out,"F")^2/nrow(P.out)^2
          tmp.mse.full=norm(P-A.impute,"F")^2/nrow(P)^2
  ...
  return(list(fpr=tmp.fpr,tpr=tmp.tpr,auc=tmp.auc,norm=tmp.norm,
              rel.err=tmp.rel.err, rel.err.full=tmp.rel.err.full,
              mse = tmp.mse, mse.full = tmp.mse.full))
}
