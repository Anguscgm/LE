  ...
  rho.result <- foreach(i = 1:M)%dopar%{
    ...
    for(k in 1:20){
      k.seq[k] <- suppressMessages(
        LEplus.egocentric.CV(A.obs,train.index,rho = 0.1,rmax=min(ceiling(SR+10),ceiling(n^(1/2))))
      )
    }
    ...
    start.time = Sys.time()
    block.fit <- suppressMessages(LEplus.egocentric(A.obs,train.index,kblock))
