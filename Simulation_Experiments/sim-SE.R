    ...
    rho.result <- foreach(i = 1:M)%dopar%{
      ...
      for(k in 1:20){
        k.seq[k] <- suppressMessages(
          SE.egocentric.CV(A,train.index,rho = 0.1,rmax=min(ceiling(SR+10),ceiling(n^(1/2))))
        )
      }
      ...
      start.time = Sys.time()
      block.fit <- SE.egocentric(A,train.index,kblock)
      # block.fit <- apply(block.fit, c(1,2), function(a) min(1,max(0,a)))
      ...
      print("Evaluating cross-validated performance ...")
